...
<div class="container">
   ...
   <table id="users-table" style="border-top: 1px solid rgb(128 128 128 / 17%);" class="table table-bordered">
      <thead>
         <tr>
            <th>ID</th>
            <th>Name</th>
...
<script>
   ...
   $('#users-table').on('click', '.edit-btn', function () {
        ...
        window.location.href = "{{ route('users.edit', ['user' => 'user_id_placeholder']) }}".replace('user_id_placeholder', userId);
   });
   ...
   var userId = $(this).data('id');
      if (confirm("Are you sure you want to delete this user?")) {
         ...
         var form = $('<form>', {
            'method': 'POST',
            'action': '{{ route('users.destroy', ':id') }}'.replace(':id', userId)
         }).append(
            $('<input>', {
                  'type': 'hidden',
                  'name': '_method',
