...
<div class="container">
   ...
   <table id="universities-table" style="border-top: 1px solid rgb(128 128 128 / 17%);" class="table table-bordered">
      <thead>
         <tr>
            <th>ID</th>
            <th>Name</th>
...
<script>
   $(function() {
       $('#universities-table').DataTable({
           ...
           columns: [
               ...
               { data: 'entrance_type', name: 'entrance_type' },
               {
                    data: 'created_at',
                    name: 'created_at',
   ...
   $('.create-btn').on('click', function () {
       window.location.href = "{{  route('universities.create')  }}";
   });
   
   $('#universities-table').on('click', '.edit-btn', function () {
        var universityId = $(this).data('id');
        window.location.href = "{{ route('universities.edit', ['university' => 'university_id_placeholder']) }}".replace('university_id_placeholder', universityId);
   });
   
   $('#universities-table').on('click', '.delete-btn', function (e) {
   e.preventDefault();
   ...
   var university_id = $(this).data('id');
    if (confirm("Are you sure you want to delete this university?")) {
        ...
        var form = $('<form>', {
            'method': 'POST',
            'action': '{{ route('universities.destroy', ':id') }}'.replace(':id', university_id)
        }).append(
